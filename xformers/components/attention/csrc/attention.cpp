...
TORCH_LIBRARY_FRAGMENT(xformers, m) {
  ...
  m.def(TORCH_SELECTIVE_SCHEMA(
      "xformers::efficient_attention2(Tensor query, Tensor key, Tensor value, bool compute_logsumexp, Tensor? attn_bias, float p, bool is_causal) -> (Tensor, Tensor, int, int)"));
  m.def(TORCH_SELECTIVE_SCHEMA(
