...
void batched_backward_bp16(BatchedBackwardParams& param, hipStream_t stream) {
  if (param.custom_mask_type == 0)
    batched_backward_mask_type_dispatched<ck::bhalf_t, 0>(param, stream);
  else if (param.custom_mask_type == 1)
    batched_backward_mask_type_dispatched<ck::bhalf_t, 1>(param, stream);
  else if (param.custom_mask_type == 2)
    batched_backward_mask_type_dispatched<ck::bhalf_t, 2>(param, stream);
  else
