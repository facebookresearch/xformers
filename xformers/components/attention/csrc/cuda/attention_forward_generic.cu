...
struct AttentionKernel {
    ...
    static void __device__ attention_kernel(
        ...
        for (int32_t iter_key_start = 0; iter_key_start < num_keys; iter_key_start += kNumWarpsPerBlock * kWarpSize) {
            ...
            for (int32_t q = warp_id; q < kQueriesPerBlock; q += kNumWarpsPerBlock) { // parallel warps
                ...
                scalar_t sp = 0;
                scalar_t my_mi = mi[q];
                static_assert(kNumWarpsPerBlock * kWarpSize % kWarpSize == 0, ".. or add a condition to loop below");
                for (int32_t key_id = lane_id; key_id < kNumWarpsPerBlock * kWarpSize; key_id += kWarpSize) { // parallel lanes
                    scalar_t si_exp = expf(si[q][key_id] - my_mi) * (key_id + iter_key_start < num_keys);
                    sp += si_exp;
                    si[q][key_id] = si_exp;
                }
                scalar_t m_prime_exp = expf(m_prime[q] - my_mi);
        ...
        int32_t iter_query_last = std::min<int32_t>((int32_t)kQueriesPerBlock, int32_t(num_queries - warp_id - query_start()));
        ...
        if (logsumexp.size(0)) {
            iter_query_last = std::min<int32_t>((int32_t)kQueriesPerBlock, int32_t(num_queries - query_start()));
            for (int64_t q = thread_id(); q < iter_query_last; q += kNumWarpsPerBlock * kWarpSize) {
                *(logsumexp.data() + query_start() + q) = m_prime[q] + std::log(s_prime[q]);
            }
        }
    }
    ...
    static __device__ void compute_dot_product_att_value(
        ...
        using ThreadblockShape = cutlass::gemm::GemmShape<kQueriesPerBlock, kNumWarpsPerBlock * kWarpSize, 8>;
        ...
        using IteratorA = cutlass::transform::threadblock::PredicatedTileIterator<
                ...
                typename MmaCore::ElementA, typename MmaCore::LayoutA, 1, typename MmaCore::IteratorThreadMapA,
        ...
        using IteratorB = cutlass::transform::threadblock::PredicatedTileIterator<
                ...
                typename MmaCore::ElementB, typename MmaCore::LayoutB, 0, typename MmaCore::IteratorThreadMapB>;


        using Mma = cutlass::gemm::threadblock::MmaPipelined<
            typename MmaCore::Shape, IteratorA, typename MmaCore::SmemIteratorA,
            IteratorB, typename MmaCore::SmemIteratorB, typename MmaCore::ElementC, typename MmaCore::LayoutC,
        ...
        cutlass::gemm::GemmCoord problem_size(
            ...
            value.size(1), // N
            std::min(kNumWarpsPerBlock * kWarpSize, value.size(0) - iter_key_start) // K
        );
        ...
        for (int blockN = 0; blockN < nBlockN; ++blockN) {
            ...
            typename Mma::Operator::IteratorC iterator_C({&output[query_start()][0], output.stride(0)}, lane_id());
            ...
            cutlass::MatrixCoord lane_offset = lane_layout.inverse(lane_id()) * cutlass::MatrixCoord(Mma::Operator::IteratorC::Policy::LaneMmaShape::kM, Mma::Operator::IteratorC::Policy::LaneMmaShape::kN);
            ...
            // Technically, we could do that on `accum`, but then we would have to wait for load to finish to
            // start the gemm calculations. Let's rather load it in parallel (software pipelining) on another register `accum2`
            accum.clear();
            accum2.clear();
            iterate_on_frag<Mma::Operator::IteratorC>(accum2, thread_offset_m, thread_offset_n, [&] (scalar_t& accum_v, int32_t m, int32_t n) {
                if (m < max_m && n < max_n) {
            ...
            // Add discounted `v_prime` (stored in `accum2`) to `accum` (which will be stored to `output`)
            {
            ...
            iterate_on_frag<Mma::Operator::IteratorC>(accum, thread_offset_m, thread_offset_n, [&] (scalar_t& accum_v, int32_t const& m, int32_t const& n) {
                if (m < max_m && n < max_n) {
    ...
    static __device__ void compute_dot_product_qk(
        ...
        */
        ...
        typename IteratorA::Params params_A(typename MmaCore::LayoutA(query.stride(0)));
        typename IteratorA::TensorRef ref_A(
            &query[query_start()][0],
            query.stride(0)
        );
        ...
        // Output results
        typename Mma::Operator::IteratorC iterator_C({&si[0][0], kSiDim1}, my_lane_id);
...
std::tuple<at::Tensor, at::Tensor, int64_t, int64_t> efficient_attention_forward_generic(
  ...
  TORCH_CHECK(!attn_bias_.has_value(), "attn_bias is not supported at the moment");
  ...
  TORCH_CHECK(
      query.scalar_type() == at::ScalarType::Float && key.scalar_type() == at::ScalarType::Float && value.scalar_type() == at::ScalarType::Float,
  ...
  at::Tensor logsumexp = at::empty({B, compute_logsumexp ? M : 0}, query.options());

  typedef float scalar_t;
  using AK = AttentionKernel<scalar_t>;
