...
def k_me_attention_fw(
    OUT, BUFFER, MAXES, WEIGHTS,    # out ptr
    Q, K, V,                # in ptrs
    M, N, L,                # dims
    ...
    SCALE: tl.constexpr,
    FUSED_NORMALIZATION: tl.constexpr,
):
    # fmt: on
    ...
    # This is positively horrible, we store then load directly
    # DEBUG - this was a tentative workaround for compiler issues, should not be needed
    buffer_ptrs = BUFFER + rm[:, None] * N + rn[None, :]
    tl.store(buffer_ptrs, qkt, mask=((rm[:, None] < M) & (rn[None, :] < N)))
    qkt = tl.load(buffer_ptrs, mask=((rm[:, None] < M) & (rn[None, :] < N)), other=0.)
...
def mem_efficient_fw(q: torch.Tensor, k: torch.Tensor, v: torch.Tensor) -> torch.Tensor:
    ...
    fused_normalization = _FUSED_NORMALIZATION and tiles_n < 2
    ...
    buffer_n = torch.empty((M, N), dtype=q.dtype, device=q.device)

    if not fused_normalization:
    ...
    for i_b in range(B):
        ...
        k_me_attention_fw[grid](
            out_n, buffer_n, maxes_n, weights_n,    # outputs
            ...
            BLOCK_L=BLOCK_L,
            SCALE=1. / math.sqrt(L),
            FUSED_NORMALIZATION=fused_normalization,
            num_warps=1
        )
        # fmt: onx
