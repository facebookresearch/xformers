...
struct FmhaFwdKernel {
  ...
  struct FmhaFwdCommonKargs {
    ...
    // for MQA/GQA, nhead could be different. This parameter is nhead_q /
    // nhead_k if this param is larger than 1, indicate MQA/GQA case
  ...
  using Kargs = std::
      conditional_t<kIsGroupMode, FmhaFwdGroupModeKargs, FmhaFwdBatchModeKargs>;
  ...
  __device__ void operator()(Kargs kargs) const {
    ...
    if constexpr (kIsGroupMode) {
      ...
      // # of required blocks is different in each groups, terminate unnecessary
      // blocks earlier
      ...
      if constexpr (kStoreLSE) {
        batch_offset_lse =
            static_cast<long_index_t>(i_batch) * kargs.batch_stride_lse;
      }
      batch_offset_o =
          static_cast<long_index_t>(i_batch) * kargs.batch_stride_o;
    }
    ...
    const auto v_dram = [&]() {
      if constexpr (ck::is_same_v<VLayout, ck::tensor_layout::gemm::RowMajor>) {
        ...
        return pad_tensor_view(
            v_dram_transposed,
            make_tuple(
                Number<FmhaPipeline::kN1>{}, Number<FmhaPipeline::kK1>{}),
    ...
    auto v_dram_window = make_tile_window(
        ...
        {i_n1, 0});
    /// FIXME: Before C++20, capturing structured binding variables is not
    /// supported. Remove following copy capture of the 'i_nhead'
    ///        if compiled in C++20
    ...
    auto lse_dram_window = [&, i_nhead_ = i_nhead]() {
      ...
      if constexpr (kStoreLSE) {
        LSEDataType* lse_ptr = reinterpret_cast<LSEDataType*>(kargs.lse_ptr) +
    ...
    FmhaMask mask = [&]() {
      if constexpr (kHasMask) {
        auto res = ck::make_tuple(
            ck::index_t{0}, ck::index_t{0}, ck::index_t{0}, ck::index_t{0});
        ...
        if (kargs.window_size > 0) {
          if (kargs.mask_type == CausalMaskType::MaskDisabled) {
            ...
            bool is_topleft =
                (kargs.mask_type ==
                 CausalMaskType::MaskUpperTriangleFromTopLeft);
