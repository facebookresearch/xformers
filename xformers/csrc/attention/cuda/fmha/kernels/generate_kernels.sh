#!/bin/bash
set -ex
rm -f forward_*.cu backward_*.cu
IFS=","

# BACKWARD
kernel="BACKWARD"
kernel_lower=`echo "\$kernel" | awk '{print tolower($0)}'`
for enable_dropout in "false" "true"; do
    for aligned in "false" "true"; do
        for maxk in 64 128 ""; do
            for dtype_name in "f32" "f16" "bf16"; do
                ...
                [[ $enable_dropout = "true" ]] && s="${s}_dropout" || s="${s}"
...
EOF
                for sm in 50 70 75 80; do
                    echo "INSTANTIATE_ATTENTION_KERNEL_${kernel}_SM${sm}($dtype, $aligned, $enable_dropout$maxk_code);" >> $FNAME
                done;
            done;
        done;
    done;
