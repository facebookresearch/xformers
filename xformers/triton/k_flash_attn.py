...
def _fwd_kernel(
    ...
    for start_n in range(0, end_n, BLOCK_N):
        ...
        # -- compute qk ----
        if EVEN_N & EVEN_M:  # Just "if EVEN_N" leads to race condition
            if EVEN_HEADDIM:
...
def _bwd_store_dk_dv(
    ...
    # BUG: In the case of EVEN_N=True and EVEN_M=False,
...
def _bwd_kernel_one_col_block(
    ...
    for start_m in range(begin_m, num_block_m * BLOCK_M, BLOCK_M):
        ...
        # compute dv
        # BUG: if EVEN_M=True and EVEN_HEADDIM=False, if we call
        ...
        # Tace condition when headdim=48/96, and dq, dk are wrong.
...
def _flash_attn_forward(q, k, v, bias=None, causal=False, softmax_scale=None):
    ...
    grid = lambda META: (triton.cdiv(seqlen_q, META["BLOCK_M"]), batch * nheads)  # noqa
    _fwd_kernel[grid](
        q,
        k,
        v,
