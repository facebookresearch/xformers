...
__device__ typename ck::vector_type<float, vec_size>::type
...
load_v(const TData* __restrict__ data_ptr, int32_t vector_offset, TDataVec* __restrict__ load_to)
{
...
template <typename TData, typename TDataVec>
__forceinline__ __device__ void
store_v(TData* __restrict__ data_ptr, int32_t vector_offset, TDataVec value)
{
    ...
    const int32_t split_k)
{
    ...
    {
        load_v<data_t, data_vec_t>(O_splits + b * O_stride_b + m * O_stride_m + g * O_stride_g +
...
#pragma unroll
    ...
    store_v<data_t, data_vec_t>(O + b * O_stride_b + m * O_stride_m + g * O_stride_g +
                                    h * O_stride_h,
                                lane_idx,
                                global_O_data.vec);
}
...
__global__ void
efficient_attention_forward_decoder_splitk_ck_kernel(const scalar_t* __restrict__ XQ,
                                                     ...
                                                     const int32_t split_k)
{
    ...
    const int32_t tt_tail_low =
        wavefront_idx * n_loop_unroll_tail + n_unrolled_loops * dtt * (split_idx + 1);
...
#pragma unroll
        ...
        data_t* __restrict__ o_ = O_splits + XQO_base_offset + split_idx * O_stride_split;
