...
template <typename scalar_t, int32_t custom_mask_type, bool has_attn_bias, bool use_fp32_qkv_grad>
struct batched_backward_masktype_attnbias_dispatched
{
    ...
    using OutputDataType   = typename std::conditional<use_fp32_qkv_grad, F32, scalar_t>::type;
...
#define BATCHED_BACKWARD_V1_HEADDIM_SWITCH(HEAD_DIM1, HEAD_DIM2, ...)              \
  ...
  using DeviceOpInstanceTemp_V2 = ck::tensor_operation::device::DeviceBatchedMultiheadAttentionBackward_Qloop_Xdl_CShuffle_V2<
    ...
    static void Run(BatchedBackwardParams& param, hipStream_t stream)
    {
        ...
        if(param.K <= 64 && param.Kv <= 64)
        {
            ...
            constexpr ck::index_t thread_slice_length_ak1 =
                GemmOpConstantsBatchedBackward_V1::AK1 /
                GemmOpConstantsBatchedBackward_V1::ABlockTransferThreadClusterLengths_AK0_M_AK1::At(
                    I2);
            constexpr ck::index_t thread_slice_length_bk1 =
                GemmOpConstantsBatchedBackward_V1::BK1 /
                GemmOpConstantsBatchedBackward_V1::BBlockTransferThreadClusterLengths_BK0_N_BK1::At(
                    I2);

            static_assert(thread_slice_length_ak1 == thread_slice_length_bk1,
                          "ABlockTransfer and BBlockTransfer should use completely same K1 sizes "
                          "and ThreadClusterLengths!");
                ...
                ALIGN_SWITCH_2(
                    ...
                    [&] {
                        using DeviceOpInstance =
                            DeviceOpInstanceTemp_V1<kGemm1NPerBlock,
                                                    kGemm1NXdlPerWave,
                                                    kCShuffleNXdlPerWavePerShuffle,
                                                    kCShuffleBlockTransferClusterLengths,
                                                    kABBlockTransferSrcScalarPerVector,
        ...
        else
        {
            ...
            constexpr ck::index_t thread_slice_length_ak1 =
                GemmOpConstantsBatchedBackward_V2::AK1 /
                GemmOpConstantsBatchedBackward_V2::ABlockTransferThreadClusterLengths_AK0_M_AK1::At(
                    I2);
            constexpr ck::index_t thread_slice_length_bk1 =
                GemmOpConstantsBatchedBackward_V2::BK1 /
                GemmOpConstantsBatchedBackward_V2::BBlockTransferThreadClusterLengths_BK0_N_BK1::At(
                    I2);
                         ...
                         kCShuffleBlockTransferScalarPerVector_max)
            {
                ALIGN_SWITCH_2(
                    ...
                    [&] {
                        ...
                        static_assert(kB1BlockTransferSrcScalarPerVector > 0,
                                      "kB1BlockTransferSrcScalarPerVector must be positive");

                        using DeviceOpInstance =
                            DeviceOpInstanceTemp_V2<kGemm1NPerBlock,
                                                    kGemm1NXdlPerWave,
                                                    kCShuffleNXdlPerWavePerShuffle,
                                                    kCShuffleBlockTransferClusterLengths,
                                                    kABBlockTransferSrcScalarPerVector,
    ...
    static void RunWithDeviceOp(BatchedBackwardParams& param, hipStream_t stream)
    {
        std::vector<ck::index_t> q_gs_ms_ks_lengths{param.B, param.Hq, param.M, param.K};
        std::vector<ck::index_t> q_gs_ms_ks_strides{
            param.q_strides[0], param.q_strides[2], param.q_strides[1], param.q_strides[3]};

        std::vector<ck::index_t> k_gs_ns_ks_lengths{param.B, param.Hkv, param.N, param.K};
        std::vector<ck::index_t> k_gs_ns_ks_strides{
            param.k_strides[0], param.k_strides[2], param.k_strides[1], param.k_strides[3]};

        std::vector<ck::index_t> kgrad_gs_ns_ks_lengths = {param.B, param.Hq, param.N, param.K};
        std::vector<ck::index_t> kgrad_gs_ns_ks_strides = {param.tmp_grad_k_strides[0],
                                                           param.tmp_grad_k_strides[2],
                                                           param.tmp_grad_k_strides[1],
                                                           param.tmp_grad_k_strides[3]};

        std::vector<ck::index_t> v_gs_os_ns_lengths{param.B, param.Hkv, param.Kv, param.N};
        std::vector<ck::index_t> v_gs_os_ns_strides{
            param.v_strides[0], param.v_strides[2], param.v_strides[3], param.v_strides[1]};

        std::vector<ck::index_t> vgrad_gs_os_ns_lengths = {param.B, param.Hq, param.Kv, param.N};
        std::vector<ck::index_t> vgrad_gs_os_ns_strides = {param.tmp_grad_v_strides[0],
                                                           param.tmp_grad_v_strides[2],
                                                           param.tmp_grad_v_strides[3],
                                                           param.tmp_grad_v_strides[1]};

        std::vector<ck::index_t> y_gs_ms_os_lengths{param.B, param.Hq, param.M, param.Kv};
...
template <typename scalar_t, int32_t custom_mask_type, bool has_attn_bias, bool use_fp32_qkv_grad>
void run_batched_backward_masktype_attnbias_dispatched(BatchedBackwardParams& param,
                                                       hipStream_t stream)
{
    batched_backward_masktype_attnbias_dispatched<scalar_t,
                                                  custom_mask_type,
                                                  has_attn_bias,
