...
#define DISPATCH_TYPES(InDataType, func)                                                  \
    {                                                                                     \
        ...
        {                                                                                 \
            XFORMERS_CHECK(false, "Only half & bf16 input type supported at the moment"); \
        }                                                                                 \
    }
...
struct CkToAtenDtype<ck::half_t>
{
    ...
    static constexpr __host__ at::ScalarType atScalarType() { return at::ScalarType::Half; }
};
...
struct CkToAtenDtype<ck::bhalf_t>
{
    ...
    static constexpr __host__ at::ScalarType atScalarType() { return at::ScalarType::BFloat16; }
};
...
struct CkToAtenDtype<float>
{
    ...
    static constexpr __host__ at::ScalarType atScalarType() { return at::ScalarType::Float; }
};
...
#define CHECK_NOSPARSE_LASTCONTIGUOUS_CUDA(TENSOR)                          \
    XFORMERS_CHECK(TENSOR.is_cuda(), #TENSOR " must be a CUDA tensor");     \
    XFORMERS_CHECK(!TENSOR.is_sparse(), #TENSOR " must be a dense tensor"); \
    XFORMERS_CHECK(TENSOR.stride(-1) == 1, #TENSOR ": last dimension must be contiguous");
 ...
 */
inline at::Tensor
get_bias_4d_view(const at::Tensor& bias, int batch_sz, int n_heads, int n_queries, int n_keys)
{
    TORCH_CHECK(bias.size(-2) == n_queries,
                ...
                n_queries);
    TORCH_CHECK(
        bias.size(-1) == n_keys, "bias.size(-1) != n_keys: ", bias.size(-1), " != ", n_keys);
    switch(bias.dim())
    {
    ...
    case 4: // (batch_sz, n_heads, n_queries, n_keys) - do nothing
        ...
        return bias;
    default: TORCH_CHECK(false, "bias can only have ndims in {2, 3, 4}");
    }
}
