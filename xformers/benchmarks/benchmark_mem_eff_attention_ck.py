...
SHAPES = [
    ...
    (1, 4096, 16, 80),
    #(1, 16384, 16, 80),  // disabled on MI250 due to big memory requirement
    # + bs4
    (4, 4096, 16, 40),
    #(4, 16384, 16, 40),  // disabled on MI250 due to big memory requirement
    (4, 4096, 16, 80),
    #(4, 16384, 16, 80),  // disabled on MI250 due to big memory requirement
    # ParlAI model
    #(256, 4096, 16, 64), // disabled on MI250 due to big memory requirement
    # Zetta B M H K
    (8, 2048, 20, 128),
    # LLaMa 70b - mp=8/16
    ...
    *sorted(
        ...
        ## disabled K/Kv bigger than 128
        itertools.product([16], [128, 512, 1024], [16], [16, 32, 64, 128])
    ),
]
...
def mem_eff_attention_bw(shape, num_threads: int, attn_bias_cfg, dropout_p, dtype):
    B, M, H, K = shape
    _, q, k, v = create_tensors(shape, dtype, requires_grad=True)
