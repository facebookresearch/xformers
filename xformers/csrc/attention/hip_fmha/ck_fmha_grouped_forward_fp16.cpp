...
void grouped_forward_fp16(GroupedForwardParams& param, hipStream_t stream) {
  if (param.custom_mask_type == 0) {
    if (param.has_attn_bias)
      ...
      grouped_forward_masktype_attnbias_dispatched<ck::half_t, 0, false>::Run(
          param, stream);
    ...
    if (param.has_attn_bias)
      ...
      grouped_forward_masktype_attnbias_dispatched<ck::half_t, 1, false>::Run(
          param, stream);
    ...
    if (param.has_attn_bias)
      ...
      grouped_forward_masktype_attnbias_dispatched<ck::half_t, 2, false>::Run(
          param, stream);
  } else
