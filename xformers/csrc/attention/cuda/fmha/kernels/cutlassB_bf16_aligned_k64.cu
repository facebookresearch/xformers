...
fmha_cutlassB_bf16_aligned_k64_sm80(typename AttentionBackwardKernel<cutlass::arch::Sm80, cutlass::bfloat16_t, true, false, 64>::Params p) {
  ...
  AttentionBackwardKernel<cutlass::arch::Sm80, cutlass::bfloat16_t, true, false, 64>::attention_kernel(p);
  return;
#endif
#endif
    printf(
        "FATAL: kernel `fmha_cutlassB_bf16_aligned_k64_sm80` is for sm80-sm90, but was built for sm%d\n",
