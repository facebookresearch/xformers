...
extern template void run_batched_infer_masktype_attnbias_dispatched<
    ck::half_t,
    0>(BatchedForwardParams& param, hipStream_t stream);

extern template void run_batched_infer_masktype_attnbias_dispatched<
    ck::half_t,
    1>(BatchedForwardParams& param, hipStream_t stream);

extern template void run_batched_infer_masktype_attnbias_dispatched<
    ck::half_t,
    2>(BatchedForwardParams& param, hipStream_t stream);

void batched_infer_fp16(BatchedForwardParams& param, hipStream_t stream) {
  if (param.custom_mask_type == 0)
    run_batched_infer_masktype_attnbias_dispatched<ck::half_t, 0>(
        param, stream);
  else if (param.custom_mask_type == 1)
    run_batched_infer_masktype_attnbias_dispatched<ck::half_t, 1>(
        param, stream);
  else if (param.custom_mask_type == 2)
    run_batched_infer_masktype_attnbias_dispatched<ck::half_t, 2>(
        param, stream);
  else
