...
template <typename scalar_t, bool has_causal_mask, bool has_attn_bias, ck::index_t HDim>
void run_grouped_forward_causalmask_attnbias_dispatched(GroupedForwardParams& param,
                                                        hipStream_t stream)
{
    grouped_forward_causalmask_attnbias_dispatched<scalar_t, has_causal_mask, has_attn_bias, HDim>::
        Run(param, stream);
};
