...
void grouped_backward_fp16(GroupedBackwardParams& param, hipStream_t stream) {
  if (param.custom_mask_type == 0)
    grouped_backward_mask_type_dispatched<ck::half_t, 0>(param, stream);
  else if (param.custom_mask_type == 1)
    grouped_backward_mask_type_dispatched<ck::half_t, 1>(param, stream);
  else if (param.custom_mask_type == 2)
    grouped_backward_mask_type_dispatched<ck::half_t, 2>(param, stream);
  else
