...
void grouped_infer_fp16(GroupedForwardParams& param, hipStream_t stream) {
  BOOL_SWITCH_1(param.has_attn_bias, HAS_ATTN_BIAS, [&] {
    if (param.custom_mask_type == 0)
      grouped_infer_masktype_attnbias_dispatched<ck::half_t, 0, HAS_ATTN_BIAS>::
          Run(param, stream);
    else if (param.custom_mask_type == 1)
      grouped_infer_masktype_attnbias_dispatched<ck::half_t, 1, HAS_ATTN_BIAS>::
          Run(param, stream);
    else if (param.custom_mask_type == 2)
      grouped_infer_masktype_attnbias_dispatched<ck::half_t, 2, HAS_ATTN_BIAS>::
          Run(param, stream);
    else
