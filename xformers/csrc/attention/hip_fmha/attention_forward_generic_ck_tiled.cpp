...
*/
std::tuple<at::Tensor, at::Tensor, int64_t, int64_t> efficient_attention_forward_ck(
    ...
    const c10::optional<int64_t> window_size)
{
    ...
    if(use_dropout)
    {
        ...
        */
        throw std::runtime_error("drop-out is currently not implemented by ck-tiled!");
    }
    ...
    { // input is batched
        ...
        if(!batched_forward_params.use_dropout && !batched_forward_params.compute_logsumexp)
        {
    ...
    { // input is grouped
        ...
        if(!grouped_forward_params.use_dropout && !grouped_forward_params.compute_logsumexp)
        {
            if(inDataType == at::ScalarType::Half)
            {
...
{
    m.impl(TORCH_SELECTIVE_NAME("xformers::efficient_attention_forward_ck"),
