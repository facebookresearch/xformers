...
fmha_cutlassB_f16_notaligned_k128_sm50(typename AttentionBackwardKernel<cutlass::arch::Sm50, cutlass::half_t, false, false, 128>::Params p) {
  ...
  AttentionBackwardKernel<cutlass::arch::Sm50, cutlass::half_t, false, false, 128>::attention_kernel(p);
  return;
#endif
#endif
    printf(
        "FATAL: kernel `fmha_cutlassB_f16_notaligned_k128_sm50` is for sm50-sm70, but was built for sm%d\n",
...
fmha_cutlassB_f16_notaligned_k128_sm70(typename AttentionBackwardKernel<cutlass::arch::Sm70, cutlass::half_t, false, false, 128>::Params p) {
  ...
  AttentionBackwardKernel<cutlass::arch::Sm70, cutlass::half_t, false, false, 128>::attention_kernel(p);
  return;
#endif
#endif
    printf(
        "FATAL: kernel `fmha_cutlassB_f16_notaligned_k128_sm70` is for sm70-sm75, but was built for sm%d\n",
        int(__CUDA_ARCH__ + 0) / 10);
#endif
}
__global__ void __launch_bounds__(
...
fmha_cutlassB_f16_notaligned_k128_sm75(typename AttentionBackwardKernel<cutlass::arch::Sm75, cutlass::half_t, false, false, 128>::Params p) {
  ...
  AttentionBackwardKernel<cutlass::arch::Sm75, cutlass::half_t, false, false, 128>::attention_kernel(p);
  return;
#endif
#endif
    printf(
        "FATAL: kernel `fmha_cutlassB_f16_notaligned_k128_sm75` is for sm75-sm80, but was built for sm%d\n",
        int(__CUDA_ARCH__ + 0) / 10);
#endif
}
__global__ void __launch_bounds__(
